...
async function main() {
  ...
  const sqlPath = resolve(process.cwd(), "scripts/apply-square-banners-v1.sql");
      ...
      const tables = await client.query<{ table_name: string }>(
        `SELECT table_name
         ...
         WHERE table_schema = 'public'
           AND table_name = 'square_banners'`,
      );
      if (tables.rows.length === 0) {
        throw new Error("square_banners table missing after migration");
      }
      ...
      const policies = await client.query<{ policyname: string }>(
        `SELECT policyname
         ...
         WHERE schemaname = 'public'
           AND tablename = 'square_banners'`,
      );
      const policyNames = new Set(policies.rows.map((row) => row.policyname));
      for (const name of ["square_banners_select_active", "square_banners_admin_all"]) {
        if (!policyNames.has(name)) {
          throw new Error(`missing RLS policy: ${name}`);
        }
      }

      console.log("OK square_banners table and RLS policies exist");
    } finally {
      await client.end();
    }
  });

  console.log("\nSquare banners V1 migration applied successfully.");
}
