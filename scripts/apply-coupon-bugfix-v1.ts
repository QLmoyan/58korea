...
async function inspectState(client: pg.Client) {
  const generateSource = await client.query<{ prosrc: string }>(
    `SELECT prosrc
     FROM pg_proc
     JOIN pg_namespace n ON n.oid = pg_proc.pronamespace
     WHERE n.nspname = 'public'
       AND proname = 'generate_redeem_code'`,
  );
  ...
  return {
    generateSource: generateSource.rows[0]?.prosrc ?? "",
...
async function main() {
  ...
  const sqlPath = resolve(process.cwd(), "scripts/apply-coupon-bugfix-v1.sql");
  ...
  try {
    ...
    if (
      state.rpcFunctions.length < 3 ||
      !state.generateSource.includes("random() * 10")
    ) {
      throw new Error("coupon bugfix migration incomplete after apply");
    }

    const sampleCode = await client.query<{ code: string }>(
      `SELECT public.generate_redeem_code() AS code`,
    );
    const code = sampleCode.rows[0]?.code ?? "";
    if (!/^\d{8}$/.test(code)) {
      throw new Error(`generate_redeem_code should return 8 digits, got: ${code}`);
    }
    console.log(`sample redeem code: ${code}`);
  } finally {
    await client.end();
  }
  ...
  console.log("\nCoupon bugfix V1 migration applied successfully.");
}
