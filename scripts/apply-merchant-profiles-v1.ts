...
async function inspectTable(client: pg.Client) {
  const rls = await client.query<{ relrowsecurity: boolean }>(
    `SELECT c.relrowsecurity
     ...
     WHERE n.nspname = 'public' AND c.relname = 'merchant_profiles'`,
  );

  const policies = await client.query<{ policyname: string; cmd: string }>(
    `SELECT policyname, cmd
     FROM pg_policies
     WHERE schemaname = 'public' AND tablename = 'merchant_profiles'
     ORDER BY policyname`,
  );

  const authorIdColumn = await client.query<{ column_name: string }>(
    `SELECT column_name
  ...
  return {
    ...
    hasAuthorIdColumn: authorIdColumn.rows.length > 0,
  };
}
...
async function main() {
  ...
  const sqlPath = resolve(process.cwd(), "scripts/apply-merchant-profiles-v1.sql");
  ...
  try {
    ...
    console.log(`posts.author_id exists: ${state.hasAuthorIdColumn}`);
    for (const policy of state.policies) {
      console.log(`- ${policy.policyname}: ${policy.cmd}`);
    }

    if (!state.rlsEnabled || state.policies.length < 4 || !state.hasAuthorIdColumn) {
      throw new Error("merchant_profiles migration incomplete after apply");
    }
  } finally {
    await client.end();
  }
  ...
  const { data: activeMerchant, error: merchantError } = await anon
  ...
  if (merchantError) {
    throw new Error(`anon cannot read active merchant_profiles: ${merchantError.message}`);
  }

  console.log(
    activeMerchant
      ? `OK anon can read active merchant: ${activeMerchant.business_name}`
      : "WARN no active merchant profile found",
  );

  const { error: insertError } = await anon.from("merchant_profiles").insert({
  ...
  if (
    ...
    console.log("OK anon cannot INSERT merchant_profiles");
  } else {
    throw new Error(
      `anon should not insert merchant_profiles (got: ${insertError?.message ?? "no error"})`,
    );
  }

  console.log("\nMerchant profiles V1 migration applied successfully.");
}
