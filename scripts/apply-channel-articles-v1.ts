...
async function main() {
  ...
  const sqlPath = resolve(process.cwd(), "scripts/apply-channel-articles-v1.sql");
      ...
      const tables = await client.query<{ table_name: string }>(
        `SELECT table_name
         ...
         WHERE table_schema = 'public'
           AND table_name IN ('channels', 'channel_articles')`,
      );
      const tableNames = new Set(tables.rows.map((row) => row.table_name));
      for (const name of ["channels", "channel_articles"]) {
        if (!tableNames.has(name)) {
          throw new Error(`table missing after migration: ${name}`);
        }
      }

      const channels = await client.query<{ count: string }>(
        `SELECT COUNT(*)::text AS count FROM public.channels WHERE is_active = true`,
      );
      const channelCount = Number(channels.rows[0]?.count ?? 0);
      if (channelCount < 6) {
        throw new Error(`expected at least 6 seed channels, got ${channelCount}`);
      }

      console.log(`OK channels (${channelCount} active), channel_articles tables exist`);
    } finally {
      await client.end();
    }
  });

  console.log("\nChannel articles V1 migration applied successfully.");
}
