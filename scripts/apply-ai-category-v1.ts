...
async function main() {
  ...
  const sqlPath = resolve(process.cwd(), "scripts/apply-ai-category-v1.sql");
    ...
    try {
      ...
      for (const name of [
        ...
        if (!columnNames.has(name)) {
          throw new Error(`posts column missing after migration: ${name}`);
        }
      }

      console.log("OK posts category columns exist");
      console.log("OK legacy categories migrated (住房→房屋, 搭子→其他)");
    } finally {
      await client.end();
    }
  });

  console.log("\nAI category V1 migration applied successfully.");
}
