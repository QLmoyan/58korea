...
async function inspectTable(client: pg.Client, tableName: string) {
...
async function main() {
  ...
  const sqlPath = resolve(process.cwd(), "scripts/apply-post-engagement-v1.sql");
  const migrationSql = readFileSync(sqlPath, "utf8");
  const connectionString = await withFirstWorkingConnection(databaseUrl, (url) =>
    applyMigration(url, migrationSql),
  );
  ...
  try {
    for (const tableName of ["post_likes", "post_favorites"]) {
      const state = await inspectTable(client, tableName);
      console.log(`\n=== ${tableName} ===`);
      ...
      if (!state.rlsEnabled || state.policies.length < 3) {
        throw new Error(`${tableName} RLS/policies incomplete after migration`);
      }
    }
  } finally {
    await client.end();
  ...
  if (publishedPost?.id) {
    const { error: likeError } = await anon.from("post_likes").insert({
    ...
    if (
      ...
      console.log("OK anon cannot INSERT post_likes");
    } else {
      throw new Error(
        `anon should not insert post_likes (got: ${likeError?.message ?? "no error"})`,
      );
    }
  }

  console.log("\nPost engagement V1 migration applied successfully.");
}
