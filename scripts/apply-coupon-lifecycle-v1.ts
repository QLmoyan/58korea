...
async function inspectState(client: pg.Client) {
  const columns = await client.query<{ column_name: string }>(
    `SELECT column_name
     ...
     WHERE table_schema = 'public'
       ...
       AND column_name IN ('expires_at', 'reminded_at')
  ...
  const statusCheck = await client.query<{ check_clause: string }>(
    `SELECT check_clause
     FROM information_schema.check_constraints
     WHERE constraint_schema = 'public'
       AND constraint_name = 'user_coupons_status_check'`,
  );

  return {
    columns: columns.rows.map((row) => row.column_name),
    rpcFunctions: rpc.rows.map((row) => row.proname),
    statusCheck: statusCheck.rows[0]?.check_clause ?? "",
  };
}
...
async function main() {
  ...
  const sqlPath = resolve(process.cwd(), "scripts/apply-coupon-lifecycle-v1.sql");
  ...
  try {
    ...
    console.log(`status check: ${state.statusCheck}`);

    if (
      state.columns.length < 2 ||
      state.rpcFunctions.length < 4 ||
      !state.statusCheck.includes("cancelled")
    ) {
      throw new Error("coupon lifecycle migration incomplete after apply");
    }
  } finally {
    await client.end();
  }
  ...
  const { error: removeError } = await anon.rpc("remove_post_linked_coupon", {
    p_post_id: 1,
  ...
  if (
    ...
    console.log("OK anon cannot remove post linked coupons");
  } else {
    throw new Error(
      `anon should not remove post linked coupons (got: ${removeError?.message ?? "no error"})`,
    );
  }

  console.log("\nCoupon lifecycle V1 migration applied successfully.");
}
