...
async function main() {
  ...
  const sqlPath = resolve(process.cwd(), "scripts/apply-post-linked-coupon-v1.sql");
      ...
      const column = await client.query<{ column_name: string }>(
        `SELECT column_name
         ...
         WHERE table_schema = 'public'
           ...
           AND column_name = 'linked_coupon_id'`,
      );

      if (column.rows.length === 0) {
        throw new Error("posts.linked_coupon_id missing after migration");
      }

      console.log("OK posts.linked_coupon_id exists");
    } finally {
      await client.end();
    }
  });

  console.log("\nPost linked coupon V1 migration applied successfully.");
}
