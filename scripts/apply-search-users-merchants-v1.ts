...
async function main() {
  ...
  const sqlPath = resolve(process.cwd(), "scripts/apply-search-users-merchants-v1.sql");
    ...
    try {
        ...
        `SELECT indexname
         ...
         WHERE schemaname = 'public'
           AND indexname IN (
             ...
             'merchant_profiles_business_name_idx'
           )`,
      );
      ...
      for (const name of [
        ...
        if (!indexNames.has(name)) {
          throw new Error(`index missing after migration: ${name}`);
        }
      }

      console.log("OK search indexes exist");
    } finally {
      await client.end();
    }
  });

  console.log("\nSearch users/merchants V1 migration applied successfully.");
}
