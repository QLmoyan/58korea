...
async function main() {
  ...
  const sqlPath = resolve(process.cwd(), "scripts/apply-comment-images-v1.sql");
  ...
  await withFirstWorkingConnection(databaseUrl, async (connectionString) => {
    ...
    try {
      ...
      if (table.rows.length === 0) {
        throw new Error("comment_images table missing after migration");
      }
      ...
      const policies = await client.query<{ policyname: string }>(
        `SELECT policyname
         ...
         WHERE schemaname = 'public'
           AND tablename = 'comment_images'`,
      );
      const policyNames = new Set(policies.rows.map((row) => row.policyname));
      for (const name of [
        "comment_images_select_published",
        "comment_images_insert_own",
        "comment_images_delete_own",
        "comment_images_admin_all",
      ]) {
      ...
      console.log("OK comment_images table exists");
      console.log("OK comment_images RLS policies exist");
    } finally {
      await client.end();
    }
  });

  console.log("\nComment images V1 migration applied successfully.");
}
