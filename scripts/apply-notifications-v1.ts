...
async function main() {
  ...
  const sqlPath = resolve(process.cwd(), "scripts/apply-notifications-v1.sql");
  ...
  await withFirstWorkingConnection(databaseUrl, async (connectionString) => {
    ...
    try {
      ...
      if (table.rows.length === 0) {
        throw new Error("notifications table missing after migration");
      }
      ...
      console.log("OK notifications RLS policies exist");
    } finally {
      await client.end();
    }
  });

  console.log("\nNotifications V1 migration applied successfully.");
}
