...
async function main() {
  loadEnv();
  const databaseUrl = resolveDatabaseUrl(process.env as Record<string, string>);
  if (!databaseUrl) {
    throw new Error("Missing SUPABASE_DB_URL, DATABASE_URL, or SUPABASE_DB_PASSWORD");
  }

  const sql = readFileSync(
    resolve(process.cwd(), "scripts/apply-user-home-v1.sql"),
    "utf8",
  );
      ...
      const { rows } = await client.query<{ policyname: string }>(
        `SELECT policyname
         ...
         WHERE schemaname = 'public'
           ...
           AND policyname = 'profiles_select_public_profiles'`,
      );

      if (!rows.length) {
        throw new Error("profiles_select_public_profiles policy missing after migration");
      }

      console.log("OK profiles_select_public_profiles exists");
      console.log("\nUnified user home V1 migration applied successfully.");
    } finally {
      await client.end();
    }
  });
